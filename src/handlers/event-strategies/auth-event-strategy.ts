...
export class SignedAuthEventStrategy
  implements IEventStrategy<Event, Promise<void>>
{
  ...
  public async execute(event: Event): Promise<void> {
    ...
    const { challenge, createdAt } =
      this.webSocket.getClientAuthChallengeData();
    const verified = isValidSignedAuthEvent(event, challenge);

    const timeIsWithinBounds =
      createdAt.getTime() + permittedChallengeResponseTimeDelayMs > Date.now();
    ...
    if (verified && timeIsWithinBounds) {
      ...
      this.webSocket.emit(
        ...
        createCommandResult(event.id, true, "authentication: succeeded")
      );
      return;
    }
    ...
    this.webSocket.emit(
      WebSocketAdapterEvent.Message,
      createCommandResult(event.id, false, "authentication: failed")
    );
  }
}
