  ...
  implements IWebSocketAdapter
{
  ...
  public constructor(
    ...
    this.clientId = Buffer.from(
      this.request.headers["sec-websocket-key"] as string,
      "base64"
    ).toString("hex");
    ...
    this.client
      .on("error", (error) => {
        if (
          ...
          console.error(
            `web-socket-adapter: client ${
              this.clientId
            } (${this.getClientAddress()}) sent payload too large`
          );
        } else if (
          error.name === "RangeError" &&
          ...
          debug(
            `client ${
              this.clientId
            } (${this.getClientAddress()}) enabled compression`
          );
        } else {
          console.error(
            `web-socket-adapter: client error ${
              this.clientId
  ...
  public onHeartbeat(): void {
    if (!this.alive && !this.subscriptions.size) {
      console.error(
        `web-socket-adapter: pong timeout for client ${
          this.clientId
        } (${this.getClientAddress()})`
      );
      this.client.close();
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          console.error(
            `web-socket-adapter: abort from client ${
              this.clientId
            } (${this.getClientAddress()})`
          );
        } else if (
          ...
          if (typeof (error as any).annotate === "function") {
            ...
            console.error(
              `web-socket-adapter: malformed message from client ${
                this.clientId
              } (${this.getClientAddress()}):`,
              error.message
            );
  ...
  private async isRateLimited(client: string): Promise<boolean> {
    ...
    const hit = (period: number, rate: number) =>
      rateLimiter.hit(`${client}:message:${period}`, 1, { period, rate });
