...
export const isValidSignedAuthEvent = (
  event: Event,
  challenge: string
): boolean => {
...
export const getPublicKey = (privkey: string) => {
  ...
  publicKeyCache[privkey] = secp256k1.utils.bytesToHex(
    secp256k1.getPublicKey(privkey, true).subarray(1)
  );
...
export const encryptKind4Event =
  (senderPrivkey: string | Buffer, receiverPubkey: Pubkey) =>
...
export const getEventExpiration = (event: Event): number | undefined => {
  const [, rawExpirationTime] =
    event.tags.find(
      (tag) => tag.length >= 2 && tag[0] === EventTags.Expiration
    ) ?? [];
